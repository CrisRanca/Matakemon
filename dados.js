...
let aux_jugador2;
//Contenedor para la imagen de lanzar el dado
let contenedorImagen_jugador2=document.getElementById("lanzarDado")
...
let array3_jugador2 = [];
//Math.random para saber que dado sale al tirar
let valorDado_jugador2;
//Variable para saber si se ha colocado el dado o no
let dadoTirado_jugador2 = false;
...
//FUNCIONES
//Funcion para tirar el dado
function tirarDado_jugador2() {
    valorDado_jugador2 = Math.floor(Math.random() * 6) + 1;

    //aux para la imagen del dado
    aux_jugador2 = `url(dados/dice${valorDado_jugador2}.png)`;
    //imagen del dado en la casilla de lanzar dado
    contenedorImagen_jugador2.style.backgroundImage = `url(dados/dice${valorDado_jugador2}.png)`;

    dadoTirado_jugador2 = true;
}
...
//EVENTOS
//Evento para clickar en la casilla de lanzar dado
contenedorImagen_jugador2.addEventListener("click", () => {
    //condicional para que no se vuelva a lanzar el dado sin colocarlo
    if(dadoTirado_jugador2) return alert("profe hijodeputa comeme los huevos");
    //llamada a la función tirarDado
    tirarDado_jugador2();
});
...
fila1_jugador2.addEventListener("click", function () {
    ...
    //condicional para que en el caso de colocar el dado haga: 
    if (dadoTirado_jugador2) {
        contenedorImagen_jugador2.style.backgroundImage = "";

        //bucle para añadir al array de la fila 1 el valor del dado colocado
        for (let i = 2; i >= 0; i--) {
            if (fila_1_jugador2[i].style.backgroundImage === "") {
                fila_1_jugador2[i].style.backgroundImage = aux_jugador2;
                array1_jugador2.push(valorDado_jugador2);
                break;
            }
        }
    ...
    if(dadoTirado_jugador2){
        ...
        for (let i = 0; i < 3; i++) {
            if (fila_2_jugador2[i].style.backgroundImage === "") {
                fila_2_jugador2[i].style.backgroundImage = aux_jugador2;
                array2_jugador2.push(valorDado_jugador2);
            break;
            }
        }   
        ...
        //mostrar la puntuacion de la fila 2
        puntuacion2_jugador2.innerHTML = `<h1>${total2_jugador2}<h1>`;
        //llamada a la funcion para calcular la puntuacion total del jugador 2
        resultadofinal_jugador2();
    }
...
fila3_jugador2.addEventListener("click", function () {
    ...
    if(dadoTirado_jugador2){
        ...
        for (let i = 0; i < 3; i++) {
            ...
            fila_3_jugador2[i].style.backgroundImage = aux_jugador2;
            array3_jugador2.push(valorDado_jugador2);
            break;
            }
        }
        ...
        //mostrar la puntuacion de la fila 3
        puntuacion3_jugador2.innerHTML = `<h1>${total3_jugador2}<h1>`;
        //llamada a la funcion para calcular la puntuacion total del jugador 2
        resultadofinal_jugador2();   
    }
